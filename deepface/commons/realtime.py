...
from LaserFR.deepface.extendedmodels import Age
from LaserFR.deepface.commons import functions, realtime, distance as dst
from LaserFR.deepface.detectors import FaceDetector
...
def analysis(db_path, model_name = 'VGG-Face', detector_backend = 'opencv', distance_metric = 'cosine', enable_face_analysis = True, source = 0, time_threshold = 5, frame_threshold = 5):
	...
	pickle_file_path = os.path.join(db_path, 'embeddings.pkl')
	...
	if os.path.exists(pickle_file_path):
		...
		# Calculate the embeddings if the pickle file does not exist
		for index in tqdm(range(len(employees)), desc="Processing employees"):
			...
			embedding = [employee, img_representation]
		...
		with open(pickle_file_path, 'wb') as f:
			pickle.dump(df, f)
		print("Embeddings saved to the pickle file.")
