...
from LaserFR.deepface.basemodels import VGGFace, OpenFace, Facenet, Facenet512, FbDeepFace, ArcFace, SFace, Boosting
from LaserFR.deepface.extendedmodels import Age, Gender, Race, Emotion
from LaserFR.deepface.commons import functions, realtime, distance as dst
...
def find(img_path, db_path, model_name ='VGG-Face', distance_metric = 'cosine', model = None, enforce_detection = True, detector_backend = 'opencv', align = True, prog_bar = True, normalization = 'base', silent=False):
	...
	if os.path.isdir(db_path) == True:
		...
		if path.exists(db_path+"/"+file_name):
			...
			if not silent:
				pass# print("WARNING: Representations for images in ",db_path," folder were previously stored in ", file_name, ". If you added new instances after this file creation, then please delete this file and call find function again. It will create it again.")

			with open(db_path+'/'+file_name, 'rb') as f:
				representations = pickle.load(f)

			if not silent:
				pass # print("There are ", len(representations)," representations found in ",file_name)
		...
		if not silent: pass #print("find function lasts ",toc-tic," seconds")
